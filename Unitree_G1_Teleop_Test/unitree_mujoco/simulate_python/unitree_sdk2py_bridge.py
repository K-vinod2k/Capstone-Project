...
class LocoServer(Server):
    def __init__(self):
        super().__init__(LOCO_SERVICE_NAME)
        self._SetApiVersion(LOCO_API_VERSION)
        
        self._RegistHandler(ROBOT_API_ID_LOCO_SET_FSM_ID, self.SetFsmId, False)
        self._RegistHandler(ROBOT_API_ID_LOCO_SET_VELOCITY, self.SetVelocity, False)
        self._RegistHandler(ROBOT_API_ID_LOCO_SET_BALANCE_MODE, self.Acknowledge, False)
        self._RegistHandler(ROBOT_API_ID_LOCO_SET_SWING_HEIGHT, self.Acknowledge, False)
        self._RegistHandler(ROBOT_API_ID_LOCO_SET_STAND_HEIGHT, self.Acknowledge, False)
        self._RegistHandler(ROBOT_API_ID_LOCO_SET_ARM_TASK, self.Acknowledge, False)

    def SetFsmId(self, parameter):
        p = json.loads(parameter)
        print(f"[LocoServer] SetFsmId: {p['data']}")
        return 0, ""

    def SetVelocity(self, parameter):
        p = json.loads(parameter)
        # print(f"[LocoServer] SetVelocity: {p['velocity']} duration: {p['duration']}")
        return 0, ""
        
    def Acknowledge(self, parameter):
        return 0, ""
...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data):
        ...
        # Loco Server for High Level Control
        self.loco_server = LocoServer()
        self.loco_server.Init()
        self.loco_server.Start()
