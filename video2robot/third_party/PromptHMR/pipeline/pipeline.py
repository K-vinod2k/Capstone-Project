...
# from pipeline.camera import run_metric_slam, calibrate_intrinsics, run_slam  # Lazy load below
...
class Pipeline:
    def __init__(self, static_cam=False):
        ...
        # PromptHMR root directory (two levels up from pipeline/pipeline.py)
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

        checkpoint_dir = os.path.join(root_dir, 'data', 'pretrain')
        self.data_dict = {
        ...
        smplx_path = os.path.join(root_dir, 'data', 'body_models', 'smplx', 'SMPLX_NEUTRAL.npz')
        self.smplx = SMPLX(
            smplx_path, 
            use_pca=False, 
            flat_hand_mean=True, 
    ...
    def estimate_2d_keypoints(self,):
        vitpose_path = self.data_dict['vitpose']
        model = load_vit_model(model_path=vitpose_path)
    ...
    def camera_motion_estimation(self, static_cam = False):
                ...
                if self.cfg.focal is None and opt_intr == False:
                    try:
                        if self.cfg.calib_method == 'ba':
                            from pipeline.camera import run_slam
                            ...
                            from pipeline.camera import calibrate_intrinsics
        ...
        if static_cam:
            ...
            try:
                from pipeline.camera import run_metric_slam
                ...
                # Catch both ValueError and missing module exceptions (e.g. lietorch)
                # print("Warning: probably there is not much camera motion in the video!! setting to zero")
                cam_R = torch.eye(3)[None].repeat_interleave(len(masks), 0)
                cam_T = torch.zeros((len(masks), 3))
                print(f"SLAM error caught: {e}")
                print("Setting camera motion to zero due to failure.")
    ...
    def __call__(self, input_video, output_folder, static_cam=False, 
        ...
        # Calculate the video2robot root directory again for the export function
        root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        smplx_npz_path = os.path.join(root_dir, 'data', 'body_models', 'smplx', 'SMPLX_neutral_array_f32_slim.npz')
