...
def main():
    ...
    parser.add_argument("--model", "-m", default="veo",
                        choices=["veo", "sora", "sora-pro", "cogvideo-5b", "cogvideo-2b"],
    ...
    elif args.model.startswith("cogvideo"):
        provider = "cogvideo"
        cog_model_id = "THUDM/CogVideoX-5b" if args.model == "cogvideo-5b" else "THUDM/CogVideoX-2b"
    else:
        provider = "veo"
    ...
    elif provider == "cogvideo":
        from video2robot.video import CogVideoClient
        client = CogVideoClient(model_id=cog_model_id)
        # CogVideo generates 49 frames at 8fps ~ 6s, ignore duration arg for now or validate it
        client.generate(
            prompt=final_prompt,
            output_path=str(output_path),
        )
