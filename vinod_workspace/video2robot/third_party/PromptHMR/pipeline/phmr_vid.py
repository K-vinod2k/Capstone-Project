...
def load_video_head():
    ...
    vid_head = vid_head.eval().to(_DEVICE)
...
class PromptHMR_Video():
    ...
    def run(self, images, results, mask_prompt=True):
        ...
        for batch in dataloader:
            with autocast(_DEVICE, enabled=(_DEVICE == 'cuda')):
        ...
        for idx, k in enumerate(list(tracks.keys())):
            ...
            with torch.amp.autocast(_DEVICE, enabled=(_DEVICE == 'cuda')):
            ...
            smplx_pose_aa = torch.cat(
                [
                    ...
                    torch.zeros(seqlen, 9).to(_DEVICE),
                ], dim=-1)
