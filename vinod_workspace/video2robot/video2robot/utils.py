...
def _conda_python(env_name: str) -> str:
    """Return the explicit Python executable path for a conda environment.

    Resolves the env's own Python directly to avoid PATH shadowing by
    active virtualenvs (e.g. a .venv in the project root).
    """
    try:
        base = subprocess.check_output(
            ["conda", "info", "--base"], text=True, stderr=subprocess.DEVNULL
        ).strip()
        py = Path(base) / "envs" / env_name / "bin" / "python"
        if py.exists():
            return str(py)
    except Exception:
        pass
...
def run_in_conda(env_name: str, argv: list[str], cwd: Path, *, raise_on_error: bool = True):
    ...
    """
    # Replace bare "python" with the env's explicit Python to avoid .venv shadowing
    if argv and argv[0] == "python":
        argv = [_conda_python(env_name)] + argv[1:]
