...
class TaskManager:
    ...
    async def run_generate_video(
        ...
        try:
            ...
            if model in self.VEO_MODEL_MAP and "veo-" in model:
                cli_model = "veo"
            ...
            while True:
                ...
                if (
                    ...
                    or "[cogvideo] loading model" in line_lower
                ):
                    self._enter_stage(
                        task,
                        name="API Request",
                ...
                elif "[cogvideo] generating" in line_lower:
                    self._enter_stage(
                        task,
                        name="Generating",
                        index=2,
                        ...
                        min_progress=0.15,
                        max_progress=0.9,
                        expected_seconds=600, # Slower on CPU/MPS
                    )
                    # Use explicit step progress if available
                    marker = self._parse_progress_marker(line_str)
                    if marker:
                         self._apply_progress_marker(task, marker)
                    else:
                ...
                if "[veo] saved" in line_lower or "[sora] saved" in line_lower or "[cogvideo] saved" in line_lower:
                    self._enter_stage(
                        task,
                        name="Saving",
                        index=4,
